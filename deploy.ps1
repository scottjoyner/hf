...
function Ensure-Oc {
  ...
  try {
    $ver = oc version --client 2>$null
    Info "oc client version: $ver"
  } catch {
    Info "oc CLI is present, but version check failed."
  }
}
...
function Await-Route([string]$route,[string]$scheme="https",[string]$path="/"){
  # Avoid clashing with automatic variable $Host
  $routeHost = oc get route $route -n $Namespace -o jsonpath='{.spec.host}' 2>$null
  if (-not $routeHost) {
    Warn "Route '$route' not found yet in namespace '$Namespace'."
    return
  }
  $url = "$($scheme)://$routeHost$path"
...
function Wait-ImageStreamTag([string]$name,[string]$tag="latest",[int]$timeoutSec=180){
  Section ("Waiting for ImageStreamTag {0}:{1}" -f $name,$tag)  # avoids interpolation edge cases
  $deadline = (Get-Date).AddSeconds($timeoutSec)
  while((Get-Date) -lt $deadline){
    if ((Oc-ExitCode ("oc get istag {0}:{1} -n {2}" -f $name,$tag,$Namespace)) -eq 0){ return }
    Start-Sleep 3
  }
  throw ("ImageStreamTag {0}:{1} not available in {2} seconds" -f $name,$tag,$timeoutSec)
}
...
function Purge-InNamespace {
  ...
  # Scale down likely consumers first
  ...
  # Also purge Endpoints/Slices in case of stale objects
  Exec "oc delete endpoints --all --ignore-not-found -n $Namespace"
  Exec "oc delete endpointslice --all --ignore-not-found -n $Namespace"

  # PVCs / PVs if requested
  if ($PurgeVolumes) {
  ...
  # Wait for pods to terminate — be tolerant of "No resources found"
  Info "Waiting for pods to terminate..."
  $deadline = (Get-Date).AddMinutes(3)
  while ((Get-Date) -lt $deadline) {
    $res = Oc-Capture "oc get pods -n $Namespace --no-headers"
    # If oc returned non-zero OR the list is empty/‘No resources found’, we’re done
    if ($res.code -ne 0 -or -not $res.text -or $res.text -match 'No resources found') { break }
    Start-Sleep -Seconds 3
  }


  # Recreate PVC skeletons immediately so later applies don't race
  if ($PurgeVolumes) {
...
function Oc-Capture([string]$cmd){
  $old = $ErrorActionPreference
  try {
    $ErrorActionPreference = 'Continue'
    $out = & cmd /c $cmd 2>&1
...
# Repair: if a pre-existing endpoints/minio slipped through, remove it once
try {
  if ((Oc-ExitCode "oc get endpoints minio -n $Namespace") -eq 0) {
    # Check owner; if not owned by Service/minio, delete it
    $ep = oc -n $Namespace get endpoints minio -o json | ConvertFrom-Json
    $owned = $false
    foreach($ow in ($ep.metadata.ownerReferences | ForEach-Object { $_ })) {
      if ($ow.kind -eq "Service" -and $ow.name -eq "minio") { $owned = $true; break }
    }
    if (-not $owned) {
      Warn "Deleting stray endpoints/minio; controller will recreate"
      Exec "oc -n $Namespace delete endpoints minio"
    }
  }
  # Also clean slices (controller will recreate)
  Exec "oc -n $Namespace delete endpointslice -l kubernetes.io/service-name=minio --ignore-not-found"
} catch { }
