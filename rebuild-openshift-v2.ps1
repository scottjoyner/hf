param(
  ...
  [string]$EnvPath = ".\.env",
  [string]$ModelsCsvPath = ".\models.csv",   # <— NEW
  [switch]$Nuke,
  [switch]$PurgeVolumes,
  [switch]$RunOnce
)

# Prevent native tools (oc) from turning stderr into terminating errors
...
function Ensure-MinioSecret {
  Section "Creating secret minio-credentials (safe)"
...
function Set-Build-DockerfilePath([string]$bcName, [string]$dockerfilePath){
  ...
  if($LASTEXITCODE -ne 0){
    throw "Failed to patch bc/$bcName dockerfilePath"
  }
}
...
function Purge-InNamespace {
  ...
  # 0) Scale down known consumers
  Scale-Down-Deployments @("minio","registry","db-web","pipeline")

  # 1) Delete namespaced workload/config (don’t mix named + typed)
  ...
  # 2) PVCs and PVs (if requested)
  if ($PurgeVolumes) {
    Warn "Deleting PVCs per -PurgeVolumes"
    Exec "oc delete pvc --all --ignore-not-found -n $Namespace"

    # Delete PVs whose claimRef.namespace == $Namespace (Retain leftovers)
    try {
  ...
  # 3) Wait for pods to terminate (treat 'No resources found' as empty)
  ...
  # 4) If we purged volumes, recreate PVCs immediately so later applies don’t race
  if ($PurgeVolumes) {
...
function Await-Deployment([string]$name){
  Exec "oc rollout status deploy/$name -n $Namespace --timeout=180s"
}

function Await-Route([string]$route,[int]$port,[string]$path="/"){
...
function Oc-ExitCode([string]$cmd){
  ...
  try {
    ...
    $global:LASTEXITCODE = 0
    # Use & to avoid string parsing weirdness; redirect all output
    & cmd /c $cmd > $null 2>&1
    return $LASTEXITCODE
  } finally {
...
function Start-And-Wait-Build([string]$name){
  ...
  # Stream logs (doesn't guarantee API is updated yet)
  Exec "oc start-build $name --from-dir . --follow -n $Namespace"

  # Find the newest build name
  ...
  if ($phase -ne 'Complete') {
    Write-Host "---- build/$b logs (tail) ----" -ForegroundColor Yellow
    oc logs build/$b -n $Namespace --tail=200
    throw "Build $b did not complete successfully (phase=$phase)"
  }
}
...
if (Test-Path $ModelsCsvPath) {
  Section "Creating ConfigMap models-csv from $ModelsCsvPath"
  oc -n $Namespace delete configmap models-csv --ignore-not-found | Out-Null
  (oc -n $Namespace create configmap models-csv `
      --from-file=models.csv="$ModelsCsvPath" `
      -o yaml --dry-run=client) | oc apply -f - | Out-Null
} else {
  Warn "$ModelsCsvPath not found; pipeline will still look for /app/data/models.csv"
}
...
Section "Writing 10-minio.yaml (restricted-v2 compliant)"
@'
apiVersion: apps/v1
kind: Deployment
metadata: { name: minio }
...
Section "Writing 20-app.yaml (workloads; restricted-v2 compliant)"
@"
apiVersion: apps/v1
kind: Deployment
...
# Apply infra (MinIO) and workloads
Exec "oc apply -f .\10-minio.patched.yaml -n $Namespace"

# Ensure models.csv exists if you plan to mount it
if (Test-Path $ModelsCsvPath) {
  Section "Creating ConfigMap models-csv from $ModelsCsvPath"
  oc -n $Namespace delete configmap models-csv --ignore-not-found | Out-Null
  (oc -n $Namespace create configmap models-csv `
...
# === Creating BuildConfigs and building images in-cluster ===
Section "Creating BuildConfigs and building images in-cluster"

# ensure BCs (don’t throw if they don’t exist)
...
Set-Build-DockerfilePath -bcName "models-registry" -dockerfilePath "Dockerfile.registry"

# build and wait
Start-And-Wait-Build "models-pipeline"
Start-And-Wait-Build "models-registry"

# restart & wait AFTER images exist
foreach ($d in @("registry","db-web","pipeline")) {
  Exec "oc rollout restart deploy/$d -n $Namespace"
  Exec "oc rollout status deploy/$d -n $Namespace --timeout=180s"
}

# Routes (optional wait)
Section "Routes"
try{
  ...
  Await-Route "minio-console" 9001 "/minio/health/ready"
...
Exec "oc apply -f .\10-minio.patched.yaml -n $Namespace"
Exec "oc wait --for=condition=complete job/minio-mc-setup -n $Namespace --timeout=180s"

# Optional one-shot pipeline sequence
if ($RunOnce) {
  ...
  Exec "oc wait --for=condition=complete job/$scr -n $Namespace --timeout=15m"

  $dnl = "run-downloader-$(Get-Random)"; Exec "oc create job $dnl --from=job/run-downloader -n $Namespace"
  Exec "oc wait --for=condition=complete job/$dnl -n $Namespace --timeout=60m"

  $md  = "run-metadata-$(Get-Random)";  Exec "oc create job $md --from=job/run-metadata -n $Namespace"
  Exec "oc wait --for=condition=complete job/$md -n $Namespace --timeout=15m"
